...
iso_name=portergos
iso_label="PORTERGOS_$(date +%Y%m)_PERSISTENT"
...
portergos_add(){
######
# Add key-init add portergos gpg
arch_chroot() {
    arch-chroot ${work_dir}/${arch}/airootfs /bin/bash -c "${1}"
    #arch-chroot ${work_dir}/airootfs /bin/bash -c "${1}"
} 

    #arch_chroot "sudo pacman-key --init; sudo pacman-key --add /usr/share/pacman/keyrings/portergos.gpg; sudo pacman-key --lsign-key 4F4B983A; sudo pacman-key --populate; sudo pacman-key --refresh-keys"
    arch_chroot "pacman-key --add /usr/share/pacman/keyrings/portergos.gpg; pacman-key --lsign-key 4F4B983A"
    cp -rf /etc/pacman.d/mirrorlist ${work_dir}/${arch}/airootfs/etc/pacman.d/mirrorlist
    #cp -rf /etc/pacman.d/mirrorlist ${work_dir}/airootfs/etc/pacman.d/mirrorlist
...
make_packages() {
    ...
    portergos_add
    #mkdir -p ${work_dir}/${arch}/airootfs/usr/share/pacman/keyrings/
    #cp ${script_path}/airootfs/usr/share/pacman/keyrings/portergos.gpg.bak ${work_dir}/${arch}/airootfs/usr/share/pacman/keyrings/portergos.gpg
    #setarch ${arch} $MKARCHISO ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'pacman-key --add /usr/share/pacman/keyrings/portergos.gpg' run
    #setarch ${arch} $MKARCHISO ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'pacman-key --lsign-key 4F4B983A' run
...
make_setup_mkinitcpio() {
    ...
    ###HACKINLINUZ
    #cp -r /home/fernando/PKGBUILD/arch/linux-4.9.9-2-x86_64.pkg/* ${work_dir}/${arch}/airootfs/
    ...
    #Test adding LTS kernel to image
    ARCHISO_GNUPG_FD=${gpg_key:+17} setarch ${arch} $MKARCHISO ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-lts -g /boot/archiso-lts.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_customize_airootfs() {
    ...
    rm ${work_dir}/${arch}/airootfs/root/customize_airootfs_offline.sh
      rm ${work_dir}/${arch}/airootfs/root/customize_airootfs.sh.bak 
}
...
make_boot() {
    ...
    # Test adding LTS kernel to image
    #trying to add LTS kernel too
    #cp -r /boot/vmlinuz-linux-lts ${work_dir}/${arch}/airootfs/boot

    cp ${work_dir}/${arch}/airootfs/boot/archiso-lts.img ${work_dir}/iso/${install_dir}/boot/${arch}/archiso-lts.img
...
make_efi() {
    ...
    # Add efi support to all kernels - LTS
    #cp -r ${script_path}/efiboot/loader/entries/* ${work_dir}/iso/loader/entries/
    ...
    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-x86_64-usb-lts.conf >> ${work_dir}/iso/loader/entries/archiso-x86_64-lts.conf

    ## Entry for persistent EFI
    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-x86_64-usb.persistent.conf > ${work_dir}/iso/loader/entries/archiso-x86_64.persistent.conf

    # Add efi support to all kernels - LTS
    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-x86_64-usb-lts.persistent.conf >> ${work_dir}/iso/loader/entries/archiso-x86_64-lts.persistent.conf
...
make_efiboot() {
    ...
    # Add efi LTS kernel
    cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux-lts ${work_dir}/efiboot/EFI/archiso/vmlinuz-lts.efi
    cp ${work_dir}/${arch}/airootfs/boot/archiso-lts.img ${work_dir}/efiboot/EFI/archiso/archiso-lts.img
    ...
    # Add efi support to all kernels - LTS
    #cp -r ${script_path}/efiboot/loader/entries/* ${work_dir}/efiboot/loader/entries/
    ...
    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-x86_64-cd-lts.conf >> ${work_dir}/efiboot/loader/entries/archiso-x86_64-lts.conf

    ## Entry for persistent EFI
    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-x86_64-cd.persistent.conf > ${work_dir}/efiboot/loader/entries/archiso-x86_64.persistent.conf

    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-x86_64-cd-lts.persistent.conf >> ${work_dir}/efiboot/loader/entries/archiso-x86_64-lts.persistent.conf
...
#before building add extra packages
    #cp -r ./airootfs/etc/pacman.d/gnupg/* ${work_dir}/x86_64/airootfs/etc/pacman.d/gnupg
